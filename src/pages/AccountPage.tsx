...
interface Sub { status: string; current_period_end: string | null }
  ...
  useEffect(() => {
    ...
    (async () => {
      const [p, s, c, u] = await Promise.all([
        ...
        supabase.from("usage_counters").select("count").eq("user_id", user.id).eq("kind", "writing_assessment_free").maybeSingle(),
      ]);
      setProfile(p.data);
      setSub(s.data);
      ...
      <div className="container mx-auto max-w-3xl px-4 py-10 space-y-6">
        ...
        <section className="grid sm:grid-cols-3 gap-4">
          <div className="bg-card border border-border rounded-2xl p-5">
            <div className="text-xs text-muted-foreground uppercase">Subscription</div>
            <div className="mt-1 font-semibold capitalize">{sub?.status || "none"}</div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            <div className="text-xs text-muted-foreground uppercase">Essay credits</div>
